...
14. [사각지대] 배치 지정 family 가 판정 집합에 포함되지만 단독이 아닌
    ...
    검토 : 'family 미확정' 을 알리는 플래그가 필요한지. 새 플래그를 두면
    판정 결과가 바뀌므로 (call_one 플래그는 곧 FAIL) 영향 범위 확인이
    선행되어야 한다. 대안으로 01_판정요약의 VH family 열에 후보수를
    함께 표시하는 방법도 있다.

15. [불일치] 랜드마크 상태와 구조 플래그가 서로 다른 집합을 만든다.
    ...
    검토 : 구조 문법이 링커 위치를 잡을 때 check_landmark 가 찾은 위치를
    쓸지. 판정 결과가 바뀌므로(d1·d2 가 계산되고 NO_LINKER 가 사라짐)
    영향 범위 확인이 선행되어야 한다.
